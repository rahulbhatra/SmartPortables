...
function createDataTable(jsonData) {
    ...
    data.push(["Product Name", "Total Items Available Count"]);
    //Create an array of product name and an array of zipcodes
    for(var i=0; i<parsedData.length; i++) {
        var productName = parsedData[i]["productName"];
        var productCount = parsedData[i]["count"];
        data.push([productName, productCount])
     }
...
function drawChart(data) {
    ...
    var options = {
        ...
        axes: {
            x: {
              0: { side: 'top', label: 'Total Item Count In Inventory'} // Top x-axis.
            }
          },
        };
