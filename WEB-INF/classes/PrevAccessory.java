...
public class PrevAccessory extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HashMap<Long, Product> hm = new HashMap<>();
		if(CategoryName==null){
			hm.putAll(SaxParserDataStore.wearableTechnologyHashMap);
			name = "";
		}
		else
		{
		   if(CategoryName.equals("microsoft"))
		   {
			 ...
			 {
				if(ProductManufacturers.MicroSoft.equals(entry.getValue().getManufacturer()))
				 {
			...
			for(Map.Entry<Long,Product> entry : SaxParserDataStore.wearableTechnologyHashMap.entrySet())
				{
					if(ProductManufacturers.Sony.equals(entry.getValue().getManufacturer()))
				 {
					 hm.put(entry.getValue().getProductId(),entry.getValue());
				 }
				}
			...
			else if(CategoryName.equals("nintendo"))
			{
				for(Map.Entry<Long,Product> entry : SaxParserDataStore.wearableTechnologyHashMap.entrySet())
				{
					if(ProductManufacturers.Nintendo.equals(entry.getValue().getManufacturer()))
				 {
					 hm.put(entry.getValue().getProductId(),entry.getValue());
				 }
				}
	...
	//	int i = 1; int size= hm.size();
		...
		pw.print("</h2><div class='entry'><table id='bestseller'>");
		Product console1 = hm.get(ProductName);
		
		int i = 1; int size= hm.size();
		pw.print("<tr>");
		ArrayList<Integer> arr = new ArrayList<> ();
		for(Map.Entry<Integer, Integer> acc:console1.getAccessories().entrySet())
		{
	    ...
	    Product accessory= SaxParserDataStore.accessoriesHasMap.get(arr.get(j));
