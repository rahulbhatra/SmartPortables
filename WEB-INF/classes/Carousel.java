...
public class Carousel{
	...
	public String  carouselfeature(Utilities utility){
		...
		if(CategoryName==null){
			try{
			hm=MySqlDataStoreUtilities.getProductsByCategory(ProductCategory.WearableTechnology);
			name = "";
			}catch(Exception e)
			{
			...
			{	
				...
				*/
				...
				for(Map.Entry<Integer, Integer> acc:console1.getAccessories().entrySet())
				{
					...
					Product accessory= MySqlDataStoreUtilities.getProductsByCategory(ProductCategory.Accessories).get(acc.getValue());
					if (k==0 )
					{
					...
					sb.append("<li><form method='post' action='WriteReview'>"+"<input type='hidden' name='name' value='"+accessory.getName()+"'>"+
							"<input type='hidden' name='type' value='accessories'>"+
							"<input type='hidden' name='maker' value='"+accessory.getManufacturer().toString()+"'>"+
							"<input type='hidden' name='access' value='"+oi.getName()+"'>"+
